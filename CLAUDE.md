...
| `src/models/NMMR/NMMR_loss.py` MAR branch (Pass 2) | §4.4 imputed risk R̂_MAR(θ) with r̃_i(θ) = δ_W r_i(θ) + (1−δ_W) m̂_θ(L⁺) |
...
In **Pass 1**, the `use_mar_modified=true` branch is a placeholder that calls the upstream `NMMR_loss` unchanged — it just exists to thread the MAR data tuple through the trainer without crashing. Algorithmic changes land in Pass 2 (loss) and Pass 3 (ATE).
